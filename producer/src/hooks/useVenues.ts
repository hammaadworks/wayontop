...
export function useVenues() {
    ...
    const loadVenues = useCallback(async () => {
        ...
        const {
            data,
            error
        } = await supabase.from('venues').select('id, name, key, lat, lng, zoom').order('created_at', {ascending: false});
        if (error) {
    ...
    const createVenue = async (venueData: Partial<Venue>) => {
        ...
        }]).select('id, name, key, lat, lng, zoom').single();
    ...
    const updateVenue = async (venueData: Partial<Venue>): Promise<boolean> => {
        ...
        const {data, error} = await supabase.from('venues').update({
            ...
            lng: venueData.lng,
            zoom: venueData.zoom || 16
        }).eq('id', venueData.id).select('id, name, key, lat, lng, zoom').single();
