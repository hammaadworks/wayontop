...
export function SponsorManager({data, setData}: Readonly<SponsorManagerProps>) {
    ...
    return (
        <Dialog>
            ...
            <DialogContent
                className="sm:max-w-md w-[95vw] max-h-[85vh] overflow-y-auto glass-panel border border-white/10 bg-black/80! backdrop-blur-3xl text-white shadow-2xl p-0 gap-0">
                ...
                <div className="p-4 sm:p-6 space-y-4">
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Select value={sponsorForm.poi_id}
                                            onValueChange={v => setSponsorForm(s => ({...s, poi_id: v || undefined}))}>
                                        <SelectTrigger
                                            className="bg-black/50 border-white/20 text-white font-medium"><SelectValue
                                            placeholder="Select a Node"/></SelectTrigger>
                                        <SelectContent className="bg-black/90 border-white/20 text-white max-h-60">
                                            {data.nodes.filter(n => n.type !== 'track' || (n.type === 'track' && n.name && n.name.trim() !== '')).map(n => (
                                                <SelectItem key={n.id} value={n.id}
                                                            className="hover:bg-white/10">{n.name || n.id}</SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input value={sponsorForm.banner_asset || ''}
                                           onChange={e => setSponsorForm(s => ({...s, banner_asset: e.target.value}))}
                                           className="bg-black/50 border-white/20 text-white"/>
                                </div>
                                <div className="space-y-2">
                                    <Label className="text-slate-300">Promo Video URL (Optional)</Label>
                                    <Input value={sponsorForm.video_asset || ''}
                                           onChange={e => setSponsorForm(s => ({...s, video_asset: e.target.value}))}
                                           className="bg-black/50 border-white/20 text-white"/>
                                </div>
                    ...
                    <div className="space-y-3">
                        {data.sponsors.map(sponsor => {
                            ...
                            return (
                                <div key={sponsor.id}
                                     className={`bg-black/40 border rounded-xl p-3 flex flex-col gap-2 ${isFilled ? 'border-amber-500/30' : 'border-slate-500/30'}`}>
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                {isFilled ? (
                                                    <span
                                                        className="px-1.5 py-0.5 rounded text-[9px] font-black bg-amber-500/20 text-amber-400 border border-amber-500/30 uppercase tracking-wider">Filled</span>
                                                ) : (
                                                    <span
                                    ...
                                    {isFilled && sponsor.tagline &&
                                        <p className="text-sm italic text-amber-300/80">"{sponsor.tagline}"</p>}
                                </div>
                            );
                        })}
