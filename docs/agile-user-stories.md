...
- **US 1.2**: As a developer, I want to set up Supabase with a `content_blobs` table (for JSON graph data) and basic analytics tables, so that the app can fetch map data and log events.
...
- **US 2.1**: As an admin, I want to view a Leaflet satellite map of Lalbagh, so I can visually map out POIs and paths.
- **US 2.2**: As an admin, I want to drop pins for POIs and draw path edges with distances, so I can construct the routing graph.
- **US 2.3**: As an admin, I want to define Sponsor Zones (poi_id + radius) visually on the map, so I can manage sponsor visibility.
- **US 2.4**: As an admin, I want to sync my mapped graph and sponsor data directly to Supabase `content_blobs` as JSON, so the consumer app can fetch the latest map immediately without a redeploy.
